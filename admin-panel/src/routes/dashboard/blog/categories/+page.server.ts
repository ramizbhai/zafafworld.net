...
export const load: PageServerLoad = async ({ cookies, fetch }) => {
    ...
    try {
        const res = await fetch(`${env.PUBLIC_API_URL || 'http://localhost:8080'}/api/v1/admin/categories`, {
            headers: {
        ...
        return { categories: [], error: 'Connection error' };
    }
};
...
export const actions: Actions = {
    create: async ({ request, cookies, fetch }) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                return fail(res.status, { error: err.message || 'Failed to create category' });
            }
    ...
    delete: async ({ request, cookies, fetch }) => {
        ...
        try {
            const res = await fetch(`${env.PUBLIC_API_URL || 'http://localhost:8080'}/api/v1/admin/categories/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${sessionToken}`,
                    'Cookie': `zafaf_admin_session=${sessionToken}`
            ...
            if (!res.ok) {
                const err = await res.json();
                return fail(res.status, { error: err.message || 'Failed to delete category' });
            }
