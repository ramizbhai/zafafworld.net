...
async fn get_client_dashboard_context(
    ...
    // 3. Inquiries
    let inquiries_rows = sqlx::query("SELECT vi.id, vi.vendor_id, vi.product_id, vi.conversation_id, vi.name, vi.phone, vi.email, vi.event_date::text, vi.guest_count, vi.message, vi.status, vi.created_at::text, vi.updated_at::text, v.name_en AS vendor_name_en, v.name_ar AS vendor_name_ar, p.title AS product_name_en, p.title AS product_name_ar FROM vendor_inquiries vi LEFT JOIN vendors v ON vi.vendor_id = v.user_id LEFT JOIN vendor_products p ON vi.product_id = p.id WHERE vi.client_id = $1 ORDER BY vi.created_at DESC")
        .bind(client_uuid).fetch_all(&mut *rls_tx.tx).await?;
