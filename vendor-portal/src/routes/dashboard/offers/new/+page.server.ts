...
export const load: PageServerLoad = async ({ parent, fetch, cookies, url }) => {
    ...
    // Fetch vendor's products for the listing selector
    let products: any[] = [];

    try {
        const productsResponse = await fetch(`${env.PUBLIC_API_URL || 'http://localhost:8080'}/api/v1/vendor/products`, {
            headers: {
        ...
        console.error('Failed to load products for listing selector:', err);
    }

    // Pre-selected listing from query param (for cross-link from Products page)
    const preSelectedListing = url.searchParams.get('listing') || '';
...
export const actions: Actions = {
    create: async ({ request, fetch, cookies }) => {
        ...
        // Helper: best-effort cleanup of an already-uploaded banner if the main call fails
        async function cleanupBanner(fileUrl: string) {
            if (!fileUrl) return;
            try {
        ...
        try {
            const apiResponse = await fetch(`${env.PUBLIC_API_URL || 'http://localhost:8080'}/api/v1/vendor/promotions`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${sessionToken}`,
            ...
            if (!apiResponse.ok || responseData.status !== 'success') {
                // Banner was already uploaded — clean it up before returning error
                if (banner_image_url) await cleanupBanner(banner_image_url);
                return fail(apiResponse.status || 400, {
                    error: responseData.message || 'Failed to create promotion.',
                    errorAr: responseData.message || 'فشل إنشاء العرض الترويجي.'
                });
            }
            ...
            if (err?.status === 303) throw err; // Let redirect pass through
            console.error('Connection error creating promotion:', err);
            // Best-effort banner cleanup on unexpected error
            if (banner_image_url) {
                try {
                    await fetch(`${env.PUBLIC_API_URL || 'http://localhost:8080'}/api/v1/vendor/promotions/cleanup-banner`, {
                        method: 'DELETE',
                        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${sessionToken}`, 'Cookie': `zafaf_vendor_session=${sessionToken}` },
                        body: JSON.stringify({ file_url: banner_image_url })
                    }).catch(() => {});
                } catch {}
            }
